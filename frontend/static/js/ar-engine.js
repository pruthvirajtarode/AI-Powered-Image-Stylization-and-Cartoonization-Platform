/**
 * ╔══════════════════════════════════════════════════════╗
 * ║  Toonify AR Engine v2.0                             ║
 * ║  Production-level Face AR - MediaPipe Face Mesh     ║
 * ║  468 landmarks · EMA smoothing · Multi-face         ║
 * ╚══════════════════════════════════════════════════════╝
 */
...
class AREngine {
    constructor(videoEl, canvasEl) {
        ...
        // Smoothed face data (multi-face ready)
        this.smoothedFaces = [];
    ...
    _onResults(results) {
        ...
        this.smoothedFaces = rawFaces.map((raw, fi) => {
            ...
            // EMA smooth every landmark into pixel space
            ...
            // Iris centres when refineLandmarks=true
            ...
            const eyeDist = Math.hypot(leftTemple.x - rightTemple.x, leftTemple.y - rightTemple.y);
            const faceScale = Math.max(eyeDist / 280, 0.3);
            ...
            const mouthOpen = (mouthH / Math.max(mouthW, 1)) > 0.28;
    ...
    startRecording(facingMode) {
        ...
        const drawFrame = () => {
            ...
            this._recordAF = requestAnimationFrame(drawFrame);
        };
