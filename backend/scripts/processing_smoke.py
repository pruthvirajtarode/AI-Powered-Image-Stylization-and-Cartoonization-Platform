...
def test_image_utilities():
    ...
    # Test stats schema consistency
    print("\nTesting image stats consistency...")
    stats_a = image_processor.get_image_stats(test_image)
    stats_b = image_processor.get_image_statistics(test_image)
    assert stats_a == stats_b, "Statistics helpers returned different payloads"
    assert set(stats_a.get('colors', {}).keys()) == {'r', 'g', 'b'}, "Stats payload must expose colors.r/g/b"
    print("  ✅ Statistics helpers are consistent")
